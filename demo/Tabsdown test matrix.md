...
| Save every authored field, reopen the menu, then clear each field back to Inherit. Change the matching global and position settings after clearing. | The saved `config:` keeps its `block-id`, removes cleared values, and immediately inherits later Style Settings changes without retaining stale declarations. |
...
| Put two byte-identical blocks in one note, configure only the second, then edit or move its source while a second settings dialog remains open. | Only the chosen block gains or updates its stable `block-id`; Save refuses a stale target instead of changing either sibling. |
