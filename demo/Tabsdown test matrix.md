...
| Change horizontal padding directly to 0/36/48 in Default and Compact; repeat with `--tabsdown-horizontal-padding: 1.5rem` in a CSS snippet. | Each value is the Default/base padding; Compact uses one-third (0/12/16 px or 0.5rem), without a separate toggle, and overflow remains usable. |
...
| Right-click one authored block across mobile, wide, and split-pane layouts (around 28rem). Confirm Position, Overflow, Density, Personality, Palette, and Alignment appear as submenus, then try Inherit, Default, and Compact density plus every personality. | The current choices are checked and save immediately; the note stays at the same scroll position; no ellipsis button is rendered; Inherit is always Compact on mobile and becomes Compact at 28rem or narrower on desktop; Compact visibly reduces tab and Rail track padding while explicit Default/Compact stays fixed; nested blocks resolve independently; touch tabs remain at least 44 × 44 px. |
