...
| Right-click one authored block and choose **Configure block…** across wide and split-pane layouts (around 28rem). Try Inherit, Default, and Compact density plus every personality. | No ellipsis button is rendered; Inherit becomes Compact only at 28rem or narrower; explicit Default/Compact stays fixed; nested blocks resolve independently; touch tabs remain at least 44 × 44 px. |
| Save every authored field, reopen the menu, then clear each field back to Inherit. Change the matching global and position settings after clearing. | The saved `config:` keeps only explicit values and disappears when every field inherits; later Style Settings changes apply without stale declarations. |
...
| Put two byte-identical blocks in one note, configure only the second, then edit or move its source while a second settings dialog remains open. | Only the chosen block changes; the already-open dialog refuses to save after any note change instead of changing either sibling. |
