...
| Right-click one authored block across wide and split-pane layouts (around 28rem). Confirm Position, Overflow, Density, Personality, Palette, and Alignment appear as submenus, then try Inherit, Default, and Compact density plus every personality. | The current choices are checked and save immediately; the note stays at the same scroll position; no ellipsis button is rendered; Inherit becomes Compact only at 28rem or narrower; explicit Default/Compact stays fixed; nested blocks resolve independently; touch tabs remain at least 44 × 44 px. |
