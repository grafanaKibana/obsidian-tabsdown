...
Tabsdown 1.4.0 adds saved per-block settings that require matching parser, configuration, and CSS support. Keep the Obsidian and Quartz plugins on matching 1.4.0-compatible releases; an older Quartz plugin may render the panels but ignore or misread the saved density, personality, palette, or alignment values. Track downstream adoption in [quartz-tabsdown issue #12](https://github.com/grafanaKibana/quartz-tabsdown/issues/12).
