...
| Group | Controls |
| --- | --- |
| General | **Size:** Compact or Default. **Personality:** Default (button), Underline, Separator, or Rail. **Overflow:** Scroll or Wrap. **Palette:** Primary or Secondary. **Accent:** custom color, or the theme accent when unset. **Alignment:** Start, Center, or Equal width. Defaults: Size = Default, Personality = Default, Overflow = Scroll, Palette = Primary, Alignment = Start. |
...
| Nested blocks | Card (default) or Flat. |
...
Top, Bottom, Left, and Right can each override personality, palette, and alignment. **Inherit defaults** keeps the global choice. These overrides apply to fenced Markdown blocks; tabs created with `mountTabs` use the global settings.
